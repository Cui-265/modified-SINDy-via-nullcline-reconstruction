function results = sindy_model_identification_generalnullcline_artificial(X, dt, dXdt, varargin)
...
for i = 1:length(du_offsets)
    for j = 1:length(dv_offsets)
        ...
        % === 手动构建Theta矩阵 [1, u, v, u^2, u^3, v^2, v^3] ===
        Theta = [ones(length(u_shifted),1), ...  % 常数项
                 u_shifted, ...                 % u
                 v_shifted, ...                 % v
                 u_shifted.^2, ...              % u^2
                 u_shifted.^3, ...              % u^3
...
% 使用固定的项名称（匹配手动构建的Theta）
term_names = {'const', 'u', 'v', 'u^2', 'u^3', 'v^2', 'v^3'};
...
function plot_top_models_combined(top_models, all_offsets, all_K, u_range, v_range, u_data, v_data)
    ...
    for i = 1:min(10, length(top_models))
        ...
        % === 更新Nullcline计算（匹配手动构建的Theta）===
        % 系数顺序: [const, u, v, u^2, u^3, v^2, v^3]
        F = Xi(1,1) + Xi(2,1)*U + Xi(3,1)*V + ...
            Xi(4,1)*U.^2 + Xi(5,1)*U.^3 + ...
            Xi(6,1)*V.^2 + Xi(7,1)*V.^3;
        
        G = Xi(1,2) + Xi(2,2)*U + Xi(3,2)*V + ...
            Xi(4,2)*U.^2 + Xi(5,2)*U.^3 + ...
            Xi(6,2)*V.^2 + Xi(7,2)*V.^3;
